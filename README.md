...
This repository is also a Claude Code plugin marketplace. The plugin carries one skill, `docs-kit`: the guide above
plus the component reference, so Claude writes docs-kit pages without being told how. It also puts `docs-kit-check`
on the PATH, which renders a page in headless Chrome and lists any error.

```text
/plugin marketplace add tannc28/docs-kit
/plugin install docs-kit@docs-kit
```

Then run `/plugin`, open **Marketplaces**, pick `docs-kit` and choose **Enable auto-update**: third-party marketplaces
start with auto-update off. From then on Claude Code checks for a new release a few minutes after each session starts.
The plugin's version is `VERSION`, so installed copies change exactly when a release is cut.

Cloud sessions (claude.ai/code) load only plugins enabled for your claude.ai account. Every release also attaches
`docs-kit-skill.zip` (the skill folder) for uploading the skill to Claude apps by hand.
...
The kit is also the npm package [`@tannc26/docs-kit`](https://www.npmjs.com/package/@tannc26/docs-kit)
(`npm install @tannc26/docs-kit`); jsDelivr serves the same files from `cdn.jsdelivr.net/npm/@tannc26/docs-kit@<version>/`.
...
| `.claude-plugin/`, `plugin/` | the Claude Code marketplace and plugin; `plugin/skills/docs-kit/` is generated by `bin/build-llms.py` |
...
4. `bin/build-llms.py --check` — the assistant guide, the skill and the plugin version match the code and `VERSION`
5. `bin/gallery-check.sh` — the gallery and the example pages render in headless Chrome and every kind of component drew
6. `claude plugin validate --strict` on the marketplace and the plugin, and `docs-kit-check` on the example pages
